...
namespace Cloudweather.Report.BusinessLogic
{
    ...
    public class WeatherReportAggregator : IWeatherReportAggregator
    {
        ...
        public async Task<WeatherReport> BuildReport(string zip, int days)
        {
            ...
            _logger.LogInformation($"Zip:{zip} over last {days}:" +
            ...
            var avarageLowTemp = tempData.Average(t => t.TempLowF);

            var WeatherReport = new WeatherReport
            {
                AverageHighF = Math.Round(averageHighTemp,1),
                AverageLowF = Math.Round(avarageLowTemp,1),
                RainfallTotalInches = totalRain,
                SnowTotalInches = totalSnow,
                ZipCode = zip,
                CreatedOn = DateTime.UtcNow
            };
            //TODO: Use cached weather report instead of making round trips
